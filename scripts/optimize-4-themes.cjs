...
async function optimizeAll() {
  ...
  for (const theme of themes) {
    try {
      ...
      console.log(`  ✓ ${theme.id}: ${(originalStats.size / 1024 / 1024).toFixed(2)}MB → ${(finalStats.size / 1024).toFixed(2)}KB (压缩 ${((1 - finalStats.size / originalStats.size) * 100).toFixed(1)}%)`);
    } catch (err) {
